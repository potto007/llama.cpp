...
void diffusion_generate_entropy_bound(llama_context *             ctx,
    ...
    if (params.kv_cache) {
        ...
        // Chunked causal PREFILL: process the prompt in ubatch-sized chunks, each writing its K/V into the
        // store and attending causally over the prefix already there. The compute buffer is sized by n_ubatch
        ...
        for (int32_t s = 0; s < n_input && prefill_ok; s += U) {
            ...
            for (int32_t i = 0; i < u; i++) {
                ...
                batch.n_seq_id[i]  = 1;
                batch.seq_id[i][0] = 0;
                // PREFILL logits are discarded (only the in-graph K,V store matters). Flag one row per chunk so
                // a capped n_outputs_max reserves a single row, not a [u, n_vocab] buffer.
